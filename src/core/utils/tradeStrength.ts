...
export function calculateTradeStrength(p: TradeStrengthParams): TradeStrengthResult {
  const signals: TradeSignal[] = [
    ...
    { name: 'EMA Aligned',  value: p.ema9 > p.ema20 && p.ema20 > p.ema50 ? 'Bull Stack' : 'No',  passed: p.ema9 > p.ema20 && p.ema20 > p.ema50,       weight: 20 },
  ...
  // Confidence is derived deterministically from the score.
  // 100% score → confidence 95, 50% → confidence 47, 0% → confidence 5.
  const confidence = Math.min(Math.round(score * 0.9 + 5), 100)
