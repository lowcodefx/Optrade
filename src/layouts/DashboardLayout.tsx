...
export function DashboardLayout({ header, leftDock, center, rightDock }: Props) {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-[#060d1a]">
      ...
      {/* ── Mobile: bottom nav ── */}
      <nav className="md:hidden flex-shrink-0 flex border-t border-[#1e293b] bg-[#0a1628] safe-area-bottom">
        {MOBILE_TABS.map(({ id, label, Icon }) => (
          <button
            key={id}
            onClick={() => setMobileTab(id)}
            className={`flex-1 flex flex-col items-center justify-center py-2.5 gap-1 text-[9px] font-semibold uppercase tracking-widest transition-colors ${
              mobileTab === id
                ? 'text-[#38bdf8] border-t-2 border-[#38bdf8]'
                : 'text-[#475569] border-t-2 border-transparent'
            }`}
          >
            <Icon size={16} />
            {label}
